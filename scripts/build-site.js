...
// Simple monogram favicon (a graduation-cap emoji) as an inline data URI —
// no separate asset file to keep track of.
const FAVICON_DATA_URI = 'data:image/svg+xml,' + encodeURIComponent(
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><text x="50%" y="50%" dominant-baseline="central" text-anchor="middle" font-size="26">🎓</text></svg>'
);
...
function metaDescriptionTag(t) {
  const bits = [t.personal.title, t.personal.summary].filter(Boolean).join(' — ');
  const desc = truncate(bits, 160);
  return desc ? `<meta name="description" content="${esc(desc)}">` : '';
}
...
<meta name="description" content="Browse tutor and professional profiles with videos, certificates, experience and contact details.">
...
<body class="profile-body" data-theme="${theme}">
  ...
  <div class="hero-band">
    <div class="container">
      <a class="breadcrumb" href="../../">&larr; All profiles</a>
    </div>
    <div class="container">
  ...
  <main id="main" class="profile-wrap">
...
function main() {
  ...
  for (const file of files) {
    ...
    profileKeys.push(tutor.profileKey);
    count++;
  }

  // SITE_URL env var lets a real deploy (or the GitHub Pages workflow) bake
  // in the real domain; falls back to a placeholder that's still valid XML/
  // robots syntax so the build never fails for lacking it.
  ...
  const urls = ['', ...profileKeys.map((k) => `p/${k}/`)];
  ...
  console.log(`robots.txt + sitemap.xml written using base URL: ${baseUrl}${process.env.SITE_URL ? '' : ' (set SITE_URL env var for your real domain)'}`);
}
